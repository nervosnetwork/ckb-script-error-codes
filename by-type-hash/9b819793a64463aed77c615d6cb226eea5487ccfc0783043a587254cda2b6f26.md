...
The length of `signature` and `pubkey hash` are incorrect.
...
There is an error while calling the syscall: `ckb_load_witness`.
...
There is an error in multisig: count of pubkey is zero.
...
Can't load precomputed secp256k1 table data from cell.
